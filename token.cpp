...
// long value;
{
  ...
  t1=heap_psi_term(4);
  t1->type=integer;
  t1->value_3=wl_mem->heap_alloc((unsigned long)sizeof(REAL));
...
// long value;
{
  ...
  t1=stack_psi_term(4);
  t1->type=integer;
  t1->value_3=wl_mem->heap_alloc((unsigned long)sizeof(REAL)); /* 12.5 */
...
// long c;
{
  ...
  /* if (sgn) f = -f; */
  tok->value_3=wl_mem->heap_alloc((unsigned long)sizeof(REAL)); /* 12.5 */
  *(REAL *)tok->value_3=f;
